...
class Helper
{
    ...
    static function notifyByFirebase($title, $body, $tokens, $data = [])        // paramete 5 =>>>> $type
    {
        ...
        $headers = array(
            'Authorization: key=AAAAwMe1nY4:APA91bEYlpptAjU5nNwKhxUgNyNUEQi6jlz9pyeJ7VsK8ZzGmpDmGhSKdLZjG_raRN83hLWuK1mB6fV1bWrsJOI6Nv-DUBYzgQYaTu-YM5yse2efOxLcoX1HurGZ5dV5hVyhdIIJQ4zM',
    ...
    static function sendNotification($model, array $notifierIds, $title, $body, $action = null ,$data_type = 'admin', $data = [],$image = null ): void
    {
        if (count($notifierIds)) {
            ...
            if ($image) {
                Photo::addPhoto($image, $notification, 'notifications', ['size' => 600, 'quality' => 50, 'relation' => 'image']);
            }

            $notification->clients()->attach($notifierIds);


            if (Token::whereIn('client_id', $notifierIds)->count()) {

                Token::whereIn('client_id', $notifierIds)->chunk(999, function ($records) use ($notification, $data, $data_type) {
...
///
    ...
    static function savePhoto(Request $request , $model , $name,$folder,$cloum)
    {
        ...
        if ($request->hasFile($name)) {
            ...
            $model->$cloum = 'uploads/'.$folder.'/' . $name;
            $model->save();

        }
    }

    static function addVideo(Request $request , $model , $name,$folder,$cloum)
    {
        ...
        if ($request->hasFile($name)) {
            ...
            $photo = $request->file($name);
            $extension = $photo->getClientOriginalExtension(); // getting image extension
            $name ='video'. time() . '' . rand(11111, 99999) . '.' . $extension; // renameing image
            $photo->move($destinationPath, $name); // uploading file to given path
            $model->$cloum = 'uploads/'.$folder.'/' . $name;
            $model->save();
    ...
    static function addRecord(Request $request , $model , $name,$folder,$cloum)
    {
